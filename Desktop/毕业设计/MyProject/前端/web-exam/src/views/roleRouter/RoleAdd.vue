...
export default {
  methods: {
    ...
    getRoleList() {
      Http.limit_alllist()
        .then(rst => {
          ...
          // 处理默认的数据选中（针对更新情况）
          this.dealDefaultSelectTree(rst.data.result)
        })
        .catch(err => {
          console.log(err)
        })
    ...
    addRole() {
      ...
      // 调用新增角色接口
      Http.role_add({ rolename: this.form.title, limits: this.selectlimits })
        .then(rst => {
          //   console.log(rst)
          this.$message({
            message: rst.data.reason,
  ...
  data() {
    return {
      id: '', //用来接收修改的角色id（mongodb的id）
      defaultselectlimits: [], //默认选中的权限（用来处理需要修改的权限，新增默认为[]数组）
      selectlimits: [], //选择后的权限列表id
      form: {
        title: '' //权限标题
      },
      rules: {
  ...
  created() {
    ...
    let { id = '', limits = '', rolename = '' } = this.$route.params

    // mysql不能存储数组自动转成字符串形式，通过split方法，用逗号将字符串分割成数组
    var newlimits = limits.split(',')

    this.defaultselectlimits = newlimits
    this.selectlimits = limits
